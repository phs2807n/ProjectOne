...
public class WorldManager : MonoBehaviour
{
    ...
    {
        Unload = 0,     // 로딩이 안되어있는 상태(해체되어 있는 상태)
        PendingUnload,  // 로딩 해체 진행중인 상태
    ...
    /// <summary>
    /// 모든 씬이 업로드 되었음을 확인하기 위한 프로퍼티(모든 씬이 Unloade상태면 true, 아니면 false) 
    /// </summary>
    ...
    /// <summary>
    /// 씬이 single로 로드할 때마다 호출될 초기화용 함수
    /// </summary>
    ...
    {
        if (sceneLoadStates[index] == SceneLoadState.Unload)    // Unload 상태인 맵만 처리
        {
            sceneLoadStates[index] = SceneLoadState.PendingLoad;    // panding 상태로 만들어서 진행 중이라고 표시
            ...
            {
                sceneLoadStates[index] = SceneLoadState.Loaded; // Loaded 상태로 변경
    ...
    /// 비동기 로딩 해체 요청 함수
    /// </summary>
    /// <param name="x">로딩해체할 맵의 x위치</param>
    /// <param name="y">로딩해체할 맵의 y위치</param>
    void RequestAsyncSceneUnload(int x, int y)
    {
        int index = GetIndex(x, y);     // 인덱스 계산
        if (sceneLoadStates[index] == SceneLoadState.Loaded)
        {
            unloadWork.Add(index);      // 로딩 완료되었을 
        }
    }
    ...
    /// 비동기 로딩 해체를 처리하는 함수
    /// </summary>
    /// <param name="index">로딩 해체할 맵의 인덱스</param>
    void AsyncSceneUnload(int index)
    {
        if (sceneLoadStates[index] == SceneLoadState.Loaded)            // 로딩이 완료된 맵만 처리
        {
            sceneLoadStates[index] = SceneLoadState.PendingUnload;      // 로딩 해체중이라고 표시
            ...
            if(scene.isLoaded)
            {
                ...
                if(sceneRoots != null && sceneRoots.Length > 0)             // 루트 오브젝트 1개 이상 있으면
                {
                    Slime[] slimes = sceneRoots[0].GetComponentsInChildren<Slime>();    // 슬라임 모두 찾아선
    ...
    private void Update()
    {
        ...
        {
            unloadWork.RemoveAll((x) => x == index);  // loadWork리스트에서 index와 같은 아이템은 모두 제거
        }
        ...
        {
            AsyncSceneUnload(index);        // 비동기 로딩 해체 시작(슬라임 되돌리기)
        }
    }

    /// <summary>
    /// 지정된 위치 주변 맵은 로딩 요청하고 그 외는 로딩 해체를 요청하는 함수
    /// </summary>
    /// <param name="mapX">지정된 맵의 x 위치</param>
    /// <param name="mapY">지정된 맵의 y 위치</param>
    void RefreshScenes(int mapX, int mapY)
    {
        int startX = Mathf.Max(0, mapX - 1);    // 시작점은 (mapX,mapY)보다 1작거나 (0,0)
        ...
        for(int y = startY; y < endY; y++)
        {
            ...
            {
                RequestAsyncSceneLoad(x, y);        // 해당 하는 것들 로딩 요처
