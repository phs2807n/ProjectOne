...
public class Test_Slime : TestBase
{
    ...
    /// <summary>
    /// 슬라임들의 머테리얼(0:아웃라인, 1:페이즈, 2:리버스페이즈)
    /// </summary>
    Material[] materials;
    ...
    private void Start()
    {   
        materials = new Material[slime.Length];     // 머티리얼 미리 찾아서 저장
    ...
    private void Update()
    {
        ...
        if(outlineThicknessChange)
        {
            float min = 0.0f;
            float max = 0.01f;
            float outlineNum = min + (max - min) * num;      // num값에 따라 최소 ~ 최대로 변경
        ...
        if(innerlineThicknessChange)
        {
            float min = 0.0f;
            float max = 0.01f;
            float innerlineNum = min + (max - min) * num;      // num값에 따라 최소 ~ 최대로 변경
