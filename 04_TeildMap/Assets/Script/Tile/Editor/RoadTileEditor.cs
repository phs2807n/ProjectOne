...
#if UNITY_EDITOR
using UnityEditor;
[CustomEditor(typeof(RoadTile))]        // RoadTile용 커스터 데이터라고 표시.
public class RoadTileEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if(roadTile != null)    // roadTile이 있어야 함
        {
            ...
            {
                EditorGUILayout.LabelField("Sprites Image Preview");    // 제목

                Texture2D texture;
                for(int i = 0; i < roadTile.sprites.Length; i++)        // sprites 에 있는 이미지를 하나씩 그리기
                {
