...
public class Slime : RecycleObject
{
    ...
    Node Current
    {
        ...
        set
        {
            if (current != value)
            {
                ...
                if (current != null)
                {
                    current.nodeType = Node.NodeType.Slime; // 새로 이동한 노드는 Slime으로 변경한기
                }
            }
        }
    }
    ...
    /// <summary>
    /// 슬라임의 이동 활성화 표시용 변수(true면 움직임, false면 안 움직임
    /// </summary>
    ...
    public void ShowOutline(bool isShow = true)
    {
        ...
        {
            mainMaterial.SetFloat(OutlineThicknessID, 0);   // 안보이는 것은 두께를 0으로 만들어서 안 보이게 만듬
        }
    }
            ...
            if (path != null && path.Count > 0 && pathWaitTime < MaxPathWaitTime)
            {
                ...
                if (!map.IsSlime(destGrid) || map.GetNode(destGrid) == Current)
                {
                    ...
                    if (direction.sqrMagnitude < 0.001f)    // 방향벡터의 길이를 확인해서 도착했는지 확인
                    {
                        ...
                        path.RemoveAt(0);                   // path의 첫벗째 위치를 제거
                    }
                    else
                    {
